      SUBROUTINE CRTLCL(IATYPE,NODTYP,IDAY,IORGIN,IASF)                  CRTLCL 
C                                                                        CRTLCL 
C ********************************************************************** CRTLCL 
C *                                                                    * CRTLCL 
C *********************  D E S C R I P T I O N  ************************ CRTLCL 
C *                                                                    * CRTLCL 
C ********************************************************************** CRTLCL 
C                                                                        CRTLCL 
C     SUBROUTINE CRTLCL CREATES ARCS FROM APOD'S AND DOS'S TO            CRTLCL 
C     LOCAL HOSPITALS.                                                   CRTLCL 
C                                                                        CRTLCL 
C ********************************************************************** CRTLCL 
C *                                                                    * CRTLCL 
C ***************************  S T A T U S  **************************** CRTLCL 
C *                                                                    * CRTLCL 
C ********************************************************************** CRTLCL 
C                                                                        CRTLCL 
C      ORIGINAL AUTHOR        : MCLAIN                                   CRTLCL 
C      ORIGINAL VERSION DATE  : 12/28/83                                 CRTLCL 
C      REVISIONS              : 12/19/84  DOCUMENTATION ADDED.           CRTLCL 
C                                2/15/85  FORMAL PARAMETERS MOVED TO     CRTLCL 
C                                         COMMON BLOCKS.                 CRTLCL 
C                                2/28/85  MISSION NETWORK REDUCED.       CRTLCL 
C                                                                        CRTLCL 
C ********************************************************************** CRTLCL 
C *                                                                    * CRTLCL 
C ********************  D E C L A R A T I O N S  *********************** CRTLCL 
C *                                                                    * CRTLCL 
C ********************************************************************** CRTLCL 
C                                                                        CRTLCL 
C  ------------------------                                              CRTLCL 
C  ---P A R A M E T E R S .                                              CRTLCL 
C  ------------------------                                              CRTLCL 
C                                                                        CRTLCL 
...
C                                                                        CRTLCL 
C  ----------------------------------                                    CRTLCL 
C  ---L O C A L   V A R I A B L E S .                                    CRTLCL 
C  ----------------------------------                                    CRTLCL 
C                                                                        CRTLCL 
      INTEGER           NODTYP,  NUMBRS,  IORGIN,  IATYPE,  IDAY,        CRTLCL 
     *                  MCCPTR,  IFRTYP,  ITOTYP                         CRTLCL 
C                                                                        CRTLCL 
      REAL              REQMNT,  UPRBND,  COST                           CRTLCL 
     *                  DIST                                             CRTLCL 
C                                                                        CRTLCL 
      DIMENSION         NUMBRS(10),  REQMNT(MCAT),  UPRBND(MCAT),        CRTLCL 
     *                  MCCPTR(MCAT)                                     CRTLCL 
C                                                                        CRTLCL 
C  ------------------------------------                                  CRTLCL 
C  ---G L O B A L   V A R I A B L E S .                                  CRTLCL 
C  ------------------------------------                                  CRTLCL 
C                                                                        CRTLCL 
...
C                                                                        CRTLCL 
C  ------------------------------                                        CRTLCL 
C  ---C O M M O N   B L O C K S .                                        CRTLCL 
C  ------------------------------                                        CRTLCL 
C                                                                        CRTLCL 
...
C                                                                        CRTLCL 
C  ----------------------------------                                    CRTLCL 
C  ---D A T A   S T A T E M E N T S .                                    CRTLCL 
C  ----------------------------------                                    CRTLCL 
C                                                                        CRTLCL 
      DATA REQMNT /MCAT * 0.0/, NUMBRS/10 * 0/, MCCPTR/MCAT * 0/         CRTLCL 
C                                                                        CRTLCL 
C ********************************************************************** CRTLCL 
C *                                                                    * CRTLCL 
C ************************  E N T R A N C E  *************************** CRTLCL 
C *                                                                    * CRTLCL 
C ********************************************************************** CRTLCL 
C                                                                        CRTLCL 
C ********************************************************************** CRTLCL 
C ***                                                                *** CRTLCL 
C *** STEP 1.                                                        *** CRTLCL 
C ***         CREATE ARCS TO LOCAL AREA HOSPITALS (ARC TYPE #16).    *** CRTLCL 
C ***         LOCAL HOSPITALS ARE THOSE WITH TRAVEL TIMES            *** CRTLCL 
C ***         LESS THAN THE MAX ALLOWABLE GROUND TRAVEL TIME (MAXGTT)*** CRTLCL 
C ***         BETWEEN THE APOD AND C9 SERVICING BASE                 *** CRTLCL 
C ***         NOTE THAT THE NODES VECTOR MUST BE CHANGED TO PROVIDE  *** CRTLCL 
C ***         THE CORRECT 'TO' NODE TO SUBROUTINE WRTARC             *** CRTLCL 
C ********************************************************************** CRTLCL 
C                                                                        CRTLCL 
      NUMBRS(1) = IORGIN                                                 CRTLCL 
      NUMBRS(3) = IDAY                                                   CRTLCL 
      ARCTXT(IATYPE)(3:5) = 'DOS'                                        CRTLCL 
      ARCTXT(IATYPE)(11:13) = 'DOS'                                      CRTLCL 
      IF(NODTYP .EQ. 6 .OR. NODTYP .EQ. 11)                              CRTLCL 
     *   THEN                                                            CRTLCL 
             NUMBRS(1) = IASF                                            CRTLCL 
             ARCTXT(IATYPE)(3:5) = 'ASF'                                 CRTLCL 
             ARCTXT(IATYPE)(11:13) = 'ASF'                               CRTLCL 
         ELSE                                                            CRTLCL 
             IF(NODTYP .EQ. 5)                                           CRTLCL 
     *          THEN                                                     CRTLCL 
                    NUMBRS(1) = IDOSDTL(IORGIN,IDOSPOD)                  CRTLCL 
                    ARCTXT(IATYPE)(3:5) = 'POD'                          CRTLCL 
                    ARCTXT(IATYPE)(11:13) = 'POD'                        CRTLCL 
             ENDIF                                                       CRTLCL 
      ENDIF                                                              CRTLCL 
      DO 100 IHSPTL = 1, MC9BAS                                          CRTLCL 
             DIST = FLOAT(IDMAT(1,IORGIN,IHSPTL))                        CRTLCL 
             IF (DIST .LE. 0.0)                                          CRTLCL 
     *          THEN                                                     CRTLCL 
                    COST = ARCCIJ(IATYPE) + DIST                         CRTLCL 
                    NUMBRS(2) = IDMAT(2,IORGIN,IHSPTL)                   CRTLCL 
                    IARCPT    = IARCS(IATYPE,NODETO)                     CRTLCL 
                    NODES(IARCPT) = IHSPDTL(IHSPTL,IPRHSP)               CRTLCL 
                    IFROM     = NODES(NODTYP)                            CRTLCL 
                    DO 110 ICAT=1,MCAT                                   CRTLCL 
                           UPRBND(ICAT)=ARCUIJ(IATYPE,ICAT)              CRTLCL 
  110              CONTINUE                                              CRTLCL 
                    IFRTYP = NODTYP                                      CRTLCL 
                    ITOTYP = 12                                          CRTLCL 
                    CALL WRTARC(IATYPE,IFROM,COST,UPRBND,MCCPTR,NUMBRS,  CRTLCL 
     *                          IFRTYP,ITOTYP)                           CRTLCL 
             ENDIF                                                       CRTLCL 
  100 CONTINUE                                                           CRTLCL 
C                                                                        CRTLCL 
C ********************************************************************** CRTLCL 
